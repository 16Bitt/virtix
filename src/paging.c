...
void switch_page(unsigned int* page_dir){
	...
	if(page_dir != root_dir){
		dump_page(page_dir);
		dump_page((unsigned int*) ((unsigned int) ((*((unsigned int*) page_dir))) & 0xFFFFF000));
		//cli();
		//hlt();
	}
...
void mmap_page(unsigned int* page_dir, unsigned int vpage, unsigned int ppage){
	vga_puts("mmap(): "); vga_puts_hex(ppage); vga_puts(" to "); vga_puts_hex(vpage);
	vga_puts("\n");
	...
	page_dir[id] = ((unsigned int) page) | 3;	//User mode, RW, present
}
